## PRO-seq hemin experiment filtering script

# Load required library
library(dplyr)

# Import hemin enhancers file
enhancers <- read.delim("~/Documents/Vihervaara/hg19/proseq/K562_hg19_hemin_enhancers.txt")

# Keep columns of interest, those with eRPK data
enhancers <- enhancers %>% select(-c("He15_FCe", "He15_log2FCe", "He15_dRPKe",
                                     "He15_Reg_pl_mn", "He30_FCe", 
                                     "He30_log2FCe", "He30_Reg_pl_mn", 
                                     "He60_FCe", "He60_log2FCe", "He60_dRPKe", 
                                     "He60_Reg_pl_mn", "He24h_FCe",
                                     "He24h_log2FCe", "He24h_dRPKe",
                                     "He24h_Reg_pl_mn", "He60R48h_FCe", 
                                     "He60R48h_log2FCe", "He60R48h_dRPKe",
                                     "He60R48h_Reg_pl_mn", "He24hR48h_FCe",
                                     "He24hR48h_log2FCe", "He24hR48h_dRPKe",
                                     "He24hR48h_Reg_pl_mn", "He15_eReg", 
                                     "He30_eReg", "He60_eReg", "He24h_eReg",
                                     "He60R48h_eReg", "He24hR48h_eReg"))
...
# Create per timepoint data frames
He0_eRPK <- enhancers %>% select(c(chr, eStart, eEnd, He0_eRPK)) %>% filter(!(He0_eRPK == 0))
He15_eRPK <- enhancers %>% select(c(chr, eStart, eEnd, He15_eRPK)) %>% filter(!(He15_eRPK == 0))
He30_eRPK <- enhancers %>% select(c(chr, eStart, eEnd, He30_eRPK)) %>% filter(!(He30_eRPK == 0))
He60_eRPK <- enhancers %>% select(c(chr, eStart, eEnd, He60_eRPK)) %>% filter(!(He60_eRPK == 0))
He24h_eRPK <- enhancers %>% select(c(chr, eStart, eEnd, He24h_eRPK)) %>% filter(!(He24h_eRPK == 0))
He48h_eRPK <- enhancers %>% select(c(chr, eStart, eEnd, mean_He48h_eRPK)) %>% filter(!(mean_He48h_eRPK == 0))

# Export per timepoint data frames as .bed files
