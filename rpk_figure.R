...
# Import overlapping enhancers .bed files, per timepoint
...
overlap_0min <- overlap_0min %>% mutate(time = "0 min")
overlap_15min <- overlap_15min %>% mutate(time = "15 min")
overlap_30min <- overlap_30min %>% mutate(time = "30 min")
overlap_60min <- overlap_60min %>% mutate(time = "60 min")
overlap_24h <- overlap_24h %>% mutate(time = "24 h")
overlap_48h <- overlap_48h %>% mutate(time = "48 h")

colnames(overlap_0min) <- c("chr_CAGE_seq", "eStart_CAGE_seq", "eEnd_CAGE_seq", 
                            "RPK_CAGE_seq", "chr_PRO_seq", "eStart_PRO_seq", 
                            "eEnd_PRO_seq", "RPK_PRO_seq", "time")
...
overlapping_enhancers <- bind_rows(overlap_0min, overlap_15min, overlap_30min, 
                                   overlap_60min, overlap_24h, overlap_48h)
